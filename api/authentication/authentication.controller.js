...
function login(req,res,next){
	...
	User.findOne({email: req.body.email}, function(err, user) {
		...
		if (!user) {
		  res.json({ success: false, message: 'Datos de usuario o contraseña incorrectos, vuelve a intentarlo.' });
		} else if (user) {
			...
			if (!user.comparePassword(req.body.password)) {
			    ...
			    return res.status(200).send({success: true, message: 'Proceso de autentificacion exitosa!',token:token});
		  	}   
		}
	});
}
